//function used to access elements on the html DOM
var $ = function (id)
{
...
// below is the list of passwords that are not good to use and should be avioded
    ...
    stringOutput = "";
    //This test checks to see the length of the password to make sure it is at least 8 characters long.
   // If the password is less than 8 characters, a message will be shared with the user letting them know of the error.
       if (stringPassword.length <8)
    {
      stringOutput= "Your password must be at 8 or more characters in length.";
    }
    else
    {
      //This test checks to make sure the password is strong enough and not listed in the bad passwords array.
      //If it is not strong enough, a message will be shared with the user letting them know of the error.
      for (var i=0; i<arrayBadPasswords.length; i++)
      {
          if (stringPassword === arrayBadPasswords[i])
          {
           stringOutput = "That password is not strong enough! Try something else!";
           break;
          }
      }
    }
    //If the password entered meets both criteria, the user will be alerted that the password is correct.
    if (stringOutput != "")
    {
...
//    $("password").value = "";
   $("password").focus();
   $("check_password").onclick = passwordCheckOld; //runs the check password function when clicked
};